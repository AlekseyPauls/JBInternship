...
def calc(current_template, dataset, args1, connectors1, args2, connectors2):
    ...
    if args2 is None:
        answ = ""
        answ += str(df[args1[0]["feature"]].sum()) + ", "
        for con in connectors1:
            if con == "and":
                answ += str(df[args1[connectors1.index(con) + 1]["feature"]].sum()) + ", "
            if con == "or":
                answ = answ[:-2] + " or " + str(
                    df[args1[connectors1.index(con) + 1]["feature"]].sum()) + ", "
        answ = answ[:-2] + "."
    ...
    answ = ""
    answ += str(df.query(s)[args1[0]["feature"]].sum()) + ", "
    for con in connectors1:
        if con == "and":
            answ += str(df.query(s)[args1[connectors1.index(con) + 1]["feature"]].sum()) + ", "
        if con == "or":
            answ = answ[:-2] + " or " + str(df.query(s)[args1[connectors1.index(con) + 1]["feature"]].sum()) + ", "
    answ = answ[:-2] + "."
